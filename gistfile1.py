...
if __name__ == '__main__':
    ...
    for i in range(0, gN//2):
        xn[i] = math.cos(2*math.pi*i*400/SAMPLE_RATE) + math.cos(2*math.pi*i * 600/SAMPLE_RATE)
    for i in range(gN//2, gN):
        xn[i] = math.cos(2*math.pi*i*600/SAMPLE_RATE) + math.cos(2*math.pi*i * 800/SAMPLE_RATE)

    # applying Goertzel on those signals, and plotting results
    delta = WINDOW_SIZE
    for i in range(0, int(gN/delta)):
        xnp = xn[i*delta:(i+1)*delta]
        xnp = xnp * np.hamming(len(xnp))

        t = np.linspace(0, 1, SAMPLE_RATE)[:WINDOW_SIZE]

        freqs, results = goertzel(xnp, SAMPLE_RATE, (400, 400))
